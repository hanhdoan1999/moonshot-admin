...
const AdvancedSearchDropdownItem = ({
  ...
  const hasChildren = isSchemaPropertyScalarArray(schemaDef, property)
    ? false
    : (schemaProperty as any)?.$ref ||
      // @ts-expect-error
      schemaProperty?.anyOf?.[0]?.$ref ||
      (schemaProperty as any)?.type === "array";
  ...
  const childResource = useMemo(() => {
    ...
    const modelRef =
    (schemaProperty as any)?.type === "array"
        ?  (schemaProperty as any)?.items?.$ref
        : // @ts-expect-error
  ...
  const onClick = (evt: MouseEvent<HTMLDivElement>) => {
    if (hasChildren) {
      ...
      onAddBlock({
        ...
        enum: ((schemaProperty as any)?.enum?.filter(Boolean) ?? []) as string[],
        defaultValue: (schemaProperty as any)?.default as
          | string
          | number
          | boolean
          | null,
        ...
        nullable: isFieldNullable((schemaProperty as any)!.type),
