  ...
  acc[field.name] = {
    OR: searchOptions
      ?.map((searchOption) => {
        ...
        if (!subField) {
          return null;
        } else if ((subField as any).__nextadmin?.kind === "scalar") {
          if ((subField as any).__nextadmin?.isList) {
            let searchTerm: string | number = search;

            if (
              (subField as any).__nextadmin?.type !== "String" &&
              !isNaN(Number(search))
            ) {
          ...
          if ((subField as any).__nextadmin?.type === "String") {
            // @ts-ignore
          ...
          if ((subField as any).__nextadmin?.type === "Int" && !isNaN(Number(search))) {
            return { [subFieldName]: Number(search) };
          }
        } else if ((subField as any).__nextadmin?.kind === "object") {
          const predicate = createNestedWherePredicate({
            field: { ...(subField as any).__nextadmin, name: subFieldName },
            options,
            search,
...
const preparePrismaListRequest = async <M extends ModelName>(
  ...
  const modelFieldNextAdminData = (modelFieldSortParam as any)?.__nextadmin;
...
export const optionsFromResource = async ({
  ...
  if (relationshipField) {
    ...
    if (modelField && (modelField as any).__nextadmin?.type !== "scalar") {
      args.resource = (modelField as any).__nextadmin?.type as ModelName;
    } else {
      console.warn(
  ...
  data.forEach((item, index) => {
    ...
    Object.keys(item).forEach((key) => {
      ...
      if (
        appDir &&
        key in listFields &&
        (listFields[key as Field<ModelName>] as any)?.formatter &&
        itemValue !== null
      ) {
        (item[key].__nextadmin_formatted = listFields[
          key as Field<ModelName>
        ] as any)?.formatter?.(itemValue ?? item[key], context);
      } else {
