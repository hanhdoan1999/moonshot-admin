...
export const getModelIdProperty = (model: ModelName) => {
  ...
  return (
    Object.keys(schemaModelProperty).find(
      (property) =>
        (schemaModelProperty[property as keyof typeof schemaModelProperty] as any)
    ...
    await Promise.all(
      fields.map(async ([name, value]) => {
        ...
        if (fieldKind === "enum") {
          ...
          if (fieldValue) {
            (fieldValue as any).enum = (fieldValue as any).enum?.map((item:any) =>
              typeof item !== "object" ? { label: item, value: item } : item
            );
          }

          if ((fieldValue as any)?.default) {
            (fieldValue as any).default =
              typeof (fieldValue as any).default !== "object"
                ? { label: (fieldValue as any).default, value: (fieldValue as any).default }
                : (fieldValue as any).default;
          }
        }
        if (fieldKind === "object") {
          ...
          if ((fieldValue as any).type !== "array") {
            //Relation One-to-Many, Many side
            (fieldValue as any).type = "string";
            (fieldValue as any).relation = modelNameRelation;
            (fieldValue as any).enum = enumeration;
            (fieldValue as any).__nextadmin = fieldNextAdmin;
          } else {
            //Relation Many-to-One
            (fieldValue as any).items = {
              type: "string",
              relation: modelNameRelation,
              enum: enumeration,
            };

            delete (fieldValue as any).anyOf;
          }
        }
      })
    );
...
export const transformData = <M extends ModelName>(
  ...
  return Object.keys(data).reduce(
    async (accP, key) => {
      ...
      const fieldKind = (field as any)?.__nextadmin?.kind;
      ...
      if (get) {
        ...
        const modelRelation = (field as any)?.__nextadmin?.type as ModelName;
        ...
        if (explicitManyToManyRelationField) {
          deepRelationModel = getDeepRelationModel(
            modelRelation as M,
            explicitManyToManyRelationField as Field<M>
          );
          deepModelRelationIdField = getModelIdProperty(
            (deepRelationModel as any)?.__nextadmin?.type as ModelName
          );
        }
        ...
        if (Array.isArray(data[key])) {
          acc[key] = data[key].map((item: any) => {
            ...
            return {
              ...
              data: {
                modelName: (deepRelationModel as any)?.__nextadmin?.type as ModelName,
              },
            };
          });
        } else {
        ...
        (field as any)?.__nextadmin?.isList &&
        (field as any).__nextadmin?.kind === "scalar"
      ) {
        acc[key] = data[key];
      } else {
        const fieldTypes = (field as any)?.__nextadmin?.type;
